...
export function parseBalanzWorkbook(
...
export class BalanzAnalyzer implements BrokerAnalyzer {
  readonly code: BrokerImportCode = "BALANZ";
  readonly name = "Balanz";
  readonly supportedFileKinds = ["XLSX" as const];
  readonly acceptMimeTypes =
    ".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";

  parse(buffer: ArrayBuffer, context: ParseContext): ImportPreviewSummary {
    const workbook = XLSX.read(buffer, { type: "array", cellDates: false });
    return parseBalanzWorkbook(workbook, context);
  }
}

export const balanzAnalyzer = new BalanzAnalyzer();
